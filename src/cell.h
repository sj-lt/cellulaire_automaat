...
public:
    ...
    cell_t(std::shared_ptr<config_t> config, cords_t coordinates);
    CELL state_ = CELL::ALIVE;
    CELL next_state_ = CELL::ALIVE;
private:
    template <class map_T,class enum_T>
    int count_neighbour_f(const map_T map, enum_T state_to_count,std::vector<std::pair<int,int>> moves);
    std::vector<std::pair<int,int>> pick_possible_moves_f(const map_t *map);
    void life_rules_f(const map_t *map);

    const std::array<std::pair<int,int>,8> moves_ = {std::make_pair
    (1,0),
    {1,1},
    {0,1},
    {-1,1},
